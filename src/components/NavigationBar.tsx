...
import { CloseButton, Dialog, DialogPanel, Disclosure, DisclosureButton, DisclosurePanel, Popover, PopoverButton, PopoverGroup, PopoverPanel, Transition } from '@headlessui/react'
...
export default function Header() {
  ...
  return (
    <header className="text-white relative z-50 bg-bg-main">
      <nav className="mx-auto flex max-w-[1440px] items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
          <Link href="/" className="text-sm font-medium leading-6">
            Home Page
          </Link>
          <Popover className="relative">
            <PopoverButton className="flex items-center gap-x-1 text-sm font-medium leading-6">
              Products
              <ChevronDownIcon className="h-3 w-3 flex-none text-blue-400" aria-hidden="true" />
            </PopoverButton>
            <Transition
              as={Fragment}
                  ...
                  {products.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-auto">
                        <CloseButton as={Link} href={item.href} className="block font-medium text-gray-900">
                          {item.name}
          ...
          <Link href="/about" className="text-sm font-medium leading-6">
            About
          </Link>
          <Link href="/about" className="text-sm font-medium leading-6">
            Contact
                  ...
                  {({ open, close }) => (
                    <div>
                      <DisclosureButton className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-medium leading-7 text-gray-900 hover:bg-gray-50">
                        Products
                        <ChevronDownIcon
                          className={classNames(open ? 'rotate-180' : '', 'h-3 w-3 flex-none')}
                          aria-hidden="true"
                        />
                      </DisclosureButton>
